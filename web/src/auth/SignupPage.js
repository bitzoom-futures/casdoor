  ...
  if (!signupItem.type || signupItem.type === "Input") {
    ...
    return (
      <Form.Item {...commonProps}>
        <Input
          variant="filled"
          placeholder={signupItem.label || signupItem.name}
        />
      </Form.Item>
    );
  } else if (
...
class SignupPage extends React.Component {
  constructor(props) {
    ...
    const isOwnReferralCode =
      sessionStorage.getItem("referralCodeOwn") === "true" &&
      storedReferralCode !== "";
    ...
    const referralCodeReadOnly =
      urlReferralCode !== "" &&
      !(isOwnReferralCode && storedReferralCode === urlReferralCode);
    this.state = {
      classes: props,
      applicationName:
  ...
  toggleReferralCodeVisible() {
    this.setState((prevState) => ({
      referralCodeVisible: !prevState.referralCodeVisible,
    }));
  }
  ...
  submitSignup(values) {
    ...
    AuthBackend.signup(values, oAuthParams).then((res) => {
      if (res.status === "ok") {
  ...
  renderFormItem(application, signupItem) {
    ...
    if (signupItem.name === "Username") {
      ...
      return (
        ...
        >
          <Select
            ...
            placeholder={
              signupItem.placeholder ||
              i18next.t("signup:Please select your tag!")
            }
            allowClear={!required}
          >
            {(signupItem.options?.length > 0
              ? signupItem.options
              : (application.tags ?? [])
            ).map((tag, index) => (
      ...
      const renderPhoneItem = () => {
        return (
          <React.Fragment>
            <Form.Item className="signup-phone" required={required}>
              <Input.Group compact>
                <Form.Item
                  name="countryCode"
                  noStyle
        ...
        <>
          <div style={{textAlign: "left", marginBottom: "4px"}}>
            <Button
              ...
              icon={
                this.state.referralCodeVisible ? (
                  <CaretUpOutlined />
                ) : (
                  <CaretDownOutlined />
                )
          ...
          <Form.Item {...tailFormItemLayout}>
            <Button
              ...
              onClick={() => this.syncReferralCodeToUrl()}
              style={{
  ...
  render() {
    ...
    if (application.signupHtml !== "") {
      return <Setting.RenderCustomHtml html={application.signupHtml} />;
    }
